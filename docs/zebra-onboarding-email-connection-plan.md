...
Clawvisor의 GBrain 및 추가 agent card 노출은 계정별 `features.proxy_lite`에 묶여
있다. 구계정에서는 이 값이 `false`이고 self-service 활성화 CTA가 없으며, Zebra에는
Clawvisor 계정 entitlement를 직접 변경할 권한이나 API가 없다. 직접 GBrain URL로
진입해도 catalog 밖 agent로 처리된다. 그래서 Zebra는 flag 변경이나 client-side
우회를 시도하지 않고, 구계정에도 남아 있는 Other agent flow와 task API로 정상
GBrain wizard와 같은 canonical 3-key 상태를 만드는 fallback을 선택했다. GBrain 카드가
보이는 계정은 기존 wizard가 계속 기본 경로다. 상세 근거는
`/Users/han/brain-offlight/docs/zebra-clawvisor-gbrain-fallback-onboarding-plan.md`를 따른다.
...
이 계약은 Source Onboarding의 Gmail 시작뿐 아니라 이메일 sidebar의 `Gmail 재연결`
버튼에도 적용된다. 두 entrypoint 모두
`startSourceOnboardingGmailIntegrationStep` →
`ZebraSourceOnboardingGmailCommand.launchPlan` 경로를 사용한다. 자동화 테스트는 prompt,
secret 비영속화, verifier 완료 gate를 다루지만 구계정 실계정 end-to-end 확인은 아직
남아 있으므로 관련 brain task는 `doing` 상태다.
