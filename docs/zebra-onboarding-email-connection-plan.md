...
`.clawvisorOnboarding`이고, checklist Email Start는 `.onboardingChecklist(.email)`이다.
...
- Sidebar Gmail Connect: 이메일 사이드바에서 Clawvisor Gmail 연결을 시작한다.
  terminal source는 `.clawvisorOnboarding`으로 둔다.
...
```text
~/Library/Application Support/zebra/onboarding/clawvisor-email-state.json
~/Library/Application Support/zebra/onboarding/clawvisor-email-setup-packets/<run-id>.md
~/Library/Application Support/zebra/onboarding/bin/zebra-clawvisor-email-onboarding
```

state file에는 secret을 저장하지 않는다. `CLAWVISOR_AGENT_TOKEN` 값, Gmail token,
Clawvisor callback secret 같은 민감값은 기록하지 않는다. state에는 `schemaVersion`,
`currentRunId`, `primaryAgent`, `selectedRuntime`, `flowKind`, `completedSections`,
`nextSection`, `waitingForUser`, `lastFailure`, `updatedAt` 정도만 둔다.
...
`ZebraClawvisorOnboardingCommand`도 Claude Code의 `/clawvisor-setup` 절차를 전제로
했다. 이제는 `flowKind`에 따라 packet content를 나눈다. [Source:
Packages/ZebraVault/Sources/ZebraVault/Email/ZebraClawvisorOnboardingCommand.swift,
2026-06-09; Source: Clawvisor Generic Agent Integration Guide,
https://raw.githubusercontent.com/clawvisor/clawvisor/main/docs/INTEGRATE_GENERIC.md,
2026-06-09]
...
`.clawvisorOnboarding`, checklist는 `.onboardingChecklist(.email)` source로 terminal을
연다.
...
- launch test: `.email` checklist launch가 `openZebraAgentTerminal` placement를
  ...
  확인한다. sidebar launch는 `.clawvisorOnboarding` source를 유지하는지 본다.
