...
| `Sources/cmuxApp.swift` | Zebra default registration | Wraps settings-file bootstrap with `ZebraAppearanceDefaults.prepareLaunchSeed()` / `finishLaunchSeed(...)` so Zebra first-launch defaults can diverge from upstream without changing `AppearanceSettings.defaultMode`. |
