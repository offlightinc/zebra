...
| `Sources/cmuxApp.swift` | Composer slot + env key | `SettingsExtension` seam: the env values `\.settingsExtensionSections` and `\.settingsExtensionViewFactory` (declared in `Sources/Zebra/Settings/ZebraSettingsExtension.swift`) feed extra sections into `SettingsRootView` + `SettingsView`. The Settings scene runs in a separate SwiftUI `Window`, so the values are also injected directly on `SettingsRootView` (the main-window `ZebraServices.injectIntoEnvironment` chain doesn't reach the Settings scene). cmux only handles the section's `String` id; the concrete Zebra view comes from `ZebraSettingsExtensionRegistry.viewFactory()`. **Every touchpoint inside the file is wrapped with `ZEBRA-SEAM:settings-extension(...)` comments** — `grep -n "ZEBRA-SEAM:settings-extension" Sources/cmuxApp.swift` lists them. Revert path: drop the marked regions, then drop `Sources/Zebra/Settings/`. |
