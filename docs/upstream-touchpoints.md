...
| File | Seam | Reason |
|---|---|---|
| `Sources/AppDelegate.swift` | DI container + first automatic welcome command seam | `ZebraServices.makeDefault(tabManager: tabManager).injectIntoEnvironment(...)` wraps the root `ContentView` once per `createMainWindow` and lets Zebra-owned socket bridges route by window-local `TabManager`. The first automatic welcome terminal runs Zebra's `zebra-agent-onboarding` script when the Zebra agent onboarding JSON says it is still needed; the manual Welcome menu keeps upstream `cmux welcome`. |
...
| `Resources/zebra-agent-onboarding` | Zebra-owned bundled script | Zebra's terminal-first primary-agent onboarding command. It scans supported agent CLIs, manages installer/launch prompts, persists onboarding state/events, and accepts `mark-ready` completion reports from the launched agent. |
