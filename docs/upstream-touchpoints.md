...
| File | Seam | Reason |
|---|---|---|
| `Sources/AppDelegate.swift` | DI container | `ZebraServices.makeDefault(tabManager: tabManager).injectIntoEnvironment(...)` wraps the root `ContentView` once per `createMainWindow` and lets Zebra-owned socket bridges route by window-local `TabManager`. |
