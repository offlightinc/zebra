...
| File | Seam | Reason |
|---|---|---|
| `Sources/AppDelegate.swift` | DI container + first-run command seam | `ZebraServices.makeDefault(tabManager: tabManager).injectIntoEnvironment(...)` wraps the root `ContentView` once per `createMainWindow` and lets Zebra-owned socket bridges route by window-local `TabManager`. The first automatic welcome command delegates to Zebra's bundled `zebra-agent-onboarding` script while the manual Welcome menu keeps upstream `cmux welcome`. |
