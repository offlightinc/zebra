...
thin-client remote 는 local brain/vault repo 가 아예 없는 것이 정상일 수 있으므로, local path absence 를 실패로 보지 않는다. PGLite/local topology 에서는 local repo path 가 실제 target 이므로 위 좁은 범위에서 못 찾으면 즉시 사용자에게 path 를 묻는다.
