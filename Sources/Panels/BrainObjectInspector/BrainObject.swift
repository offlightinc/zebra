...
struct TaskObject {
    ...
    var priority: BrainPriority?
    var owner: String?
    var reviewer: String?
    var due: BrainDate?
    var related: [BrainObjectRef]
    var blockedBy: [BrainObjectRef]
    var blockedReason: String?
    var waitingOn: String?
    var tags: [String]
    var lastUpdated: BrainDate?
    var backlinks: Int?
    var checklist: BrainChecklist?
}
...
struct GoalObject {
    ...
    var parentGoal: BrainObjectRef?
    var subgoals: [BrainObjectRef]
    var tasks: [BrainObjectRef]
    var metrics: [BrainMetric]
    var milestones: [BrainMilestone]
...
enum BrainTaskStatus: String, CaseIterable {
    ...
    static var primaryCases: [BrainTaskStatus] {
        [.backlog, .todo, .inprogress, .blocked, .done]
    }
...
struct BrainObjectRef: Hashable {
    /// Verbatim from the frontmatter: `tasks/x`, `G-2026-Q2-04`, etc.
    var raw: String
    /// Heuristic: looks like `<TYPE>-<YYYY>-…` rather than a path.
    var looksLikeId: Bool { !raw.contains("/") && raw.uppercased() == raw.prefix(1).uppercased() + raw.dropFirst() }
...
/// math against today.
struct BrainDate: Equatable {
    var date: Date
    var source: String
}

struct BrainChecklist: Equatable {
    var done: Int
    var total: Int
}

struct BrainMetric: Equatable {
    var name: String
    var from: Double
    var to: Double
    var unit: String?
...
struct BrainMilestone: Equatable {
    var name: String
    var date: BrainDate?
    var done: Bool
    var current: Bool
...
struct BrainObjectParseError: Error, Equatable {
    var line: Int
    var column: Int
    var message: String
}
...
struct BrainObjectParse {
    ...
    /// failed mid-block.
    var result: Result<BrainObject, BrainObjectParseError>
}

// MARK: - Parser entry point

enum BrainObjectParser {
    /// Parse a full markdown file into a `BrainObjectParse`.
    ///
    /// Best-effort and side-effect free; safe to run off the main actor.
    static func parse(_ source: String, filename: String) -> BrainObjectParse {
